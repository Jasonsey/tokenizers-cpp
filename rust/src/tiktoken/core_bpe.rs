// tiktoken的BPE输入：
// encoder: HashMap<Vec<u8>, usize>, 正常词表 -> idx
// special_tokens_encoder: HashMap<String, usize>, 特殊token词表 -> idx
// pattern: &str,
...
impl CoreBPE {
    ...
    pub fn encode_bytes(&self, bytes: &[u8]) -> Vec<usize> {
    ...
    pub fn encode_with_unstable(&self, text: &str, allowed_special: HashSet<&str>) -> (Vec<usize>, HashSet<Vec<usize>>) {
        let (tokens, completions) = self._encode_unstable_native(text, &allowed_special);
        (tokens, completions)
    }
    ...
    pub fn decode_single_token_bytes(&self, token: usize) -> Result<&Vec<u8>, String> {
