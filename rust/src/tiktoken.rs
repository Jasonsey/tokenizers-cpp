...
impl CoreBPE {
    ...
    fn encode_with_unstable(&self, text: &str, allowed_special: HashSet<&str>) -> (Vec<usize>, HashSet<Vec<usize>>) {
        let (tokens, completions) = self._encode_unstable_native(text, &allowed_special);
        (tokens, completions)
    }

    fn encode_single_token(&self, piece: &[u8]) -> Result<usize, dyn Error> {
    ...
    fn decode_single_token_bytes(&self, token: usize) -> Result<&Vec<u8>, dyn Error> {
